...
// Points available for a check-in submitted today (Brisbane day-of-week).
// Mon=5, Tue=4, Wed=3, Thu=2, Fri=1, Sat/Sun=0. Mirrors the tiered award
// in the trigger from migration 0011 (modulo the 30-min commit grace,
// which we deliberately don't expose in the UI).
export function pointsForToday(now = new Date()) {
  const { isoWeekday } = brisbaneParts(now);
  return isoWeekday <= 5 ? 6 - isoWeekday : 0;
}
